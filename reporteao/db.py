...
def init():
    with sqlite3.connect(archivo) as db:
        cursor = db.cursor()
        cursor.execute("CREATE TABLE IF NOT EXISTS reportes(id integer primary key autoincrement, autor, titulo, sala, descripcion, imagen, fecha, likes, estado)")
...
def crear_usuario(email, nombre, clave, nivel):
    with sqlite3.connect(archivo) as db:
        cursor = db.cursor()
